...
//! committed oracle in packages/gql/__fixtures__ (the JS implementation's
//! frozen output). The case list + options mirror the vitest matrix in
//! packages/gql/__tests__/gqlSchemaToReferences.test.ts exactly.

use std::fs;

use apitoolchain_gql::{gql_schema_to_references, Options};

fn run_case(name: &str, options: Option<Options>) {
    let fixtures =
        apitoolchain_parity::fixtures_dir(env!("CARGO_MANIFEST_DIR"), "apitoolchain-gql");
    ...
    apitoolchain_parity::assert_parity(&case, &actual);
}
