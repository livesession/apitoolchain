...
//! `apitoolchain_openapi::core::schema_object_to_property` — the same function the JS
//! impl borrows from `@xyd-js/openapi`.

use serde_json::{json, Map, Value};

use apitoolchain_openapi::core::{schema_object_to_property, VisitedRefs};
use apitoolchain_openapi::DocCtx;
...
fn json_schema_properties_to_definition_properties(schema: Option<&Value>) -> Vec<Value> {
    ...
    for name in apitoolchain_uniform::jsrt::js_object_keys(props) {
