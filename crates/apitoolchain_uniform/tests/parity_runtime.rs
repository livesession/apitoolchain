...
use apitoolchain_uniform::canon;
use apitoolchain_uniform::converters::uniform_to_input_json_schema;
use apitoolchain_uniform::plugins::{plugin_json_view, plugin_navigation};
use serde_json::{json, Value};

fn fixture(name: &str) -> std::path::PathBuf {
    apitoolchain_parity::fixtures_dir(env!("CARGO_MANIFEST_DIR"), "apitoolchain-uniform").join(name)
}
...
fn assert_case(name: &str, actual: &Value) {
    ...
    let oracle = apitoolchain_parity::read_oracle(&case);
