...
use uniform::canon;
use uniform::converters::uniform_to_input_json_schema;
use uniform::plugins::{plugin_json_view, plugin_navigation};

fn fixture(name: &str) -> std::path::PathBuf {
    parity::fixtures_dir(env!("CARGO_MANIFEST_DIR"), "uniform").join(name)
}
...
fn assert_case(name: &str, actual: &Value) {
    ...
    let oracle = parity::read_oracle(&case);
